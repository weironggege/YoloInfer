...
class YOLOV5:
    ...
    def __init__(self, modelpath, conf_thres=0.7, nms_thres=0.8):
        ...
        self.net = torch.jit.load(modelpath)
        self.net.to("cuda:0")
    ...
    def detect(self, image):
        ...
        input_data = torch.from_numpy(blob).to("cuda:0")

        outs = [o.cpu().numpy() for o in self.net(input_data)]
        print(len(outs))
...
if __name__ == "__main__":
    ...
    net = YOLOV5("models/lxfire_base_torchscript3outGpu.pt")

    srcimg = cv2.imread("imgs/00007.jpg")

    tarimg = net.detect(srcimg)

    cv2.imwrite("imgs/00007_out3_gpu.jpg", tarimg)
