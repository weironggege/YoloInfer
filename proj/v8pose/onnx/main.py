...
class YOLOV8POSE:
    ...
    def __init__(self, modelpath, conf_thres=0.7, nms_thres=0.8):
        ...
        self.skeletons = [int(x.strip()) for x in open("pindex.txt").readlines()]
    ...
    def drawPose(self, image, kps):
        ...
        for i in range(self.keyPnums):
            if kps[i * 3 + 2] > 0.5:
                cv2.circle(image, (int(kps[i*3]), int(kps[i*3+1])), 5, tuple(random.sample(range(255), 3)), 2)
        ...
        for j in range(0, len(self.skeletons), 2):
            if kps[(self.skeletons[j]-1)*3+2] > 0.5:
                ...
                cv2.line(image, pos1, pos2, tuple(random.sample(range(255), 3)), 2, cv2.LINE_AA)
    ...
    def detect(self, image):
        ...
        for pred in np.squeeze(outs[0]).transpose():
            ...
            if conf > self.confThreshold:
                ...
                for i in range(self.keyPnums):
                    kpx = (pred[i * 3 + 5] - padw) * ratiow
                    kpy = (pred[i * 3 + 6] - padh) * ratioh
                    kpv = pred[i * 3 + 7]
...
if __name__ == "__main__":
    ...
    cv2.imwrite("imgs/ppose_onnx_py.jpg", tarimg)
